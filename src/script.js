document.addEventListener("DOMContentLoaded", function () {
  ...
  if (videoPlaceholder) {
    videoPlaceholder.addEventListener("click", function () {
      ...
      alert(
        "Video would play here. Replace with actual video embed when available."
      );
  ...
  // Updated contact form handling for Netlify
    ...
    if (contactForm) {
      ...
      contactForm.addEventListener("submit", function (e) {
        ...
        if (typeof gtag === "function") {
          ...
          const email =
            document.getElementById("email")?.value || "not_provided";
          const subject =
            document.getElementById("subject")?.value || "not_provided";

          // Pseudonymize email for privacy
          const hashedEmail =
            email !== "not_provided"
              ? btoa(email).substring(0, 10)
              : "not_provided";
        ...
        submitButton.innerHTML =
          '<span class="rotating-icon"><i class="fas fa-spinner fa-spin"></i></span> Sending...';
  ...
  const howItWorksBtn = document.querySelector(
    ...
    howItWorksBtn.addEventListener("click", function (e) {
      ...
      if (howItWorksSection) {
        ...
        const sectionPosition =
          howItWorksSection.getBoundingClientRect().top + window.pageYOffset;
        window.scrollTo({
  ...
  const setupEnhancedAnalytics = () => {
    ...
    document.querySelectorAll(".feature-card").forEach((card, index) => {
      const featureName =
        card.querySelector("h3")?.innerText || `Feature ${index + 1}`;
    ...
    document
      .querySelectorAll(
        ".btn-primary, .pp-btn-primary, .btn-donate, .pp-btn-donate"
      )
      .forEach((btn) => {
    ...
    window.addEventListener("beforeunload", () => {
      const sessionDuration = Math.round(
        (new Date().getTime() - sessionStart) / 1000
      );
